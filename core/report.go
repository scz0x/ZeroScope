...
func GenerateReportJSON(outputDir string, r Report) {
	...
	fmt.Println("[✓] report.json generated at", path)
}
...
<table><tr><th>Type</th><th>Count</th><th>Size (MB)</th></tr>`
	...
	html += `</table><h2>⚠️ Suspicious .so Files</h2><ul>`
	for _, f := range r.Suspicious {
		html += fmt.Sprintf("<li class='danger'>%s</li>", f)
	}
	html += `</ul><h2>🔐 Permissions</h2><ul>`
	for _, p := range r.Permissions {
		class := ""
		if IsDangerous(p) {
			class = "class='danger'"
	...
	html += `</ul><h2>🧬 Sensitive Strings</h2><ul>`
	...
	html += fmt.Sprintf(`</ul><h2>📦 Summary</h2><ul>
<li>Detected types: %d</li>
<li>Dangerous permissions: %d</li>
<li>Sensitive strings: %d</li>
</ul><hr><p>🎯 Generated by <strong>ZeroScope</strong> — developed by scz0x<br>📡 Channel: <a href="https://t.me/SCZ0X_CH">t.me/SCZ0X_CH</a></p></body></html>`,
		len(r.FileCounts), dangerCount, len(r.SecretsFound),
	)

	out.WriteString(html)
	fmt.Println("[✓] report.html generated at", filepath.Join(outputDir, "report.html"))
}
...
func GeneratePDFReport(outputDir string, r Report) {
	...
	pdf.Cell(0, 10, "🎯 Report by ZeroScope — developed by scz0x")
	pdf.Ln(5)
	pdf.Cell(0, 10, "📡 Channel: https://t.me/SCZ0X_CH")

	outPath := filepath.Join(outputDir, "report.pdf")
	if err := pdf.OutputFileAndClose(outPath); err != nil {
		fmt.Println("[✗] Failed to write PDF:", err)
	} else {
		fmt.Println("[✓] report.pdf generated at", outPath)
	}
}